...
public class RouteController {
    ...
    @Operation(
            summary = "루트 생성",
            description = "회원이 방문한 장소 정보를 포함한 새로운 루트를 생성합니다. "
    ...
    @Operation(
            summary = "루트 목록 조회",
            description = "등록된 모든 루트 목록을 조회합니다. "
                    + "각 루트의 제목, 키워드, 등록일 등의 요약 정보가 포함됩니다."
    )
    @GetMapping
    ...
    @Operation(
            summary = "루트 상세 조회",
            description = "루트 ID를 이용해 특정 루트의 상세 정보를 조회합니다. "
                    + "루트 내 포함된 장소 목록, 리뷰 등 세부 정보가 반환됩니다."
    )
    @GetMapping("/{routeId}")
